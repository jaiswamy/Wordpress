...
function cf7_save_upload_dir( $dirs ) {
    $dirs['subdir'] = '/cf7-uploads-save';
    $dirs['path'] = $dirs['basedir'] . '/cf7-uploads-save';
    $dirs['url'] = $dirs['baseurl'] . '/cf7-uploads-save';
    return $dirs;
}
function cf7_dropfiles_upload(){
    ...
    $size = @$_REQUEST["size"];
    $type = @$_REQUEST["type"];
    $type_upload = @$_REQUEST["type_upload"];
    ...
    if( $upload ) {
        switch ($type_upload) {
            case 1:
            case 2:
                    add_filter( 'upload_dir', array($this,'cf7_save_upload_dir') );
                    $upload_dir   = wp_upload_dir();
                    $filename = wp_unique_filename( $upload_dir, $filename );

                    $cf7_dirname = $upload_dir['basedir'].'/cf7-uploads-save';
                    ...
                    remove_filter( 'upload_dir', array($this,'cf7_save_upload_dir') );
                break;
            default:
...
function cf7_dropfiles_remove(){
    ...
    $upload_dir = wp_upload_dir();
    if($type == 1 || $type == 2 ){
        $path_main = $upload_dir['basedir'] . '/cf7-uploads-save/'.$name;
    }else{
...
function cf7_add_files( $components,$current , $form  ) {
    $type = get_post_meta($current->id(),"_cf7_dropfiles",true);
    if( $type == 2) {
        //to do
    }else{
        if ( $submission = WPCF7_Submission::get_instance() ) { 
            ...
            if($type == 1 || $type == 2 ){ 
                $path_main = $upload_dir['basedir'] . '/cf7-uploads-save/';
            }else{
    ...
    $abc = '<div class="cf7-dragandrophandler-container">
              <div class="cf7-dragandrophandler" data-type="'.$file_type_pattern.'" data-size="'.$allowed_size.'" data-max="'.$max.'">
                <div class="cf7-dragandrophandler-inner">
                    <div class="cf7-text-drop">'.__($text1,"cf7_dropfiles").'</div>
                    <div class="cf7-text-or">'.__($text2,"cf7_dropfiles").'</div>
                    <div class="cf7-text-browser"><a href="#">'.__($text3,"cf7_dropfiles").'</a></div>
                </div>
                <input type="file" class="input-uploads hidden" multiple>
            </div></div>';
    $html = sprintf(
        '<div class="wpcf7-form-control-wrap %1$s">'.$abc.'<input %2$s />%3$s</div>',
...
function wpcf7_drop_messages($messages){
    return array_merge( $messages, array(
        'invalid_file_min' => array(
            'description' => __( "Count files are smaller than the minimum allowed", "cf7_dropfiles" ),
            'default' => __( "Count files are smaller than the minimum allowed", "cf7_dropfiles" )
        ),

        'invalid_file_max' => array(
            'description' => __( "Count files are smaller than the maximum allowed", "cf7_dropfiles" ),
            'default' => __( "Count files are smaller than the maximum allowed", "cf7_dropfiles" )
        ),
    ) );
}
...
function wpcf7_add_tag_generator_file() {
    ...
    $tag_generator->add( 'dropfiles', __( 'Drop files', "cf7_dropfiles" ),
...
function wpcf7_tag_generator_file( $contact_form, $args = '' ) {
    ...
    <tbody>
        ...
        <tr>
        <th scope="row"><label for="<?php echo esc_attr( $args['content'] . '-id' ); ?>"><?php echo esc_html( __( 'Min files', "cf7_dropfiles" ) ); ?></label></th>
        <td><input type="text" name="min" class="idvalue oneline option" id="<?php echo esc_attr( $args['content'] . '-min' ); ?>" /></td>
        </tr>
        
        <tr>
        <th scope="row"><label for="<?php echo esc_attr( $args['content'] . '-id' ); ?>"><?php echo esc_html( __( 'Max files', "cf7_dropfiles" ) ); ?></label></th>
